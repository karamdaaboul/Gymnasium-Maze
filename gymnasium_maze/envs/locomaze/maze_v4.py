...
Original Author of the code: Justin Fu, Updated by: Karam Daaboul
...
class Maze:
    ...
    - :attr:`dangerous_locations` - All the `(x,y)` coordinates for dangerous cells. # <<< ADDED
    ...
    * v2 & v1: legacy versions in the [D4RL](https://github.com/Farama-Foundation/D4RL).
    ...
    def __init__(
        ...
        self._dangerous_locations = []  # <<< ADDED
    ...
    # <<< ADDED
    @property
    def dangerous_locations(self) -> List[np.ndarray]:
        """Returns all the dangerous locations in continuous
        coordinates (x,y)
        """
        return self._dangerous_locations
    # >>> ADDED
...
class MazeEnv(GoalEnv):
    ...
    def _is_in_dangerous_zone(self, achieved_goal: np.ndarray) -> bool:
        """Checks if the agent is in a dangerous zone."""
        for dangerous_pos in self.maze.dangerous_locations:
            if np.linalg.norm(achieved_goal - dangerous_pos) <= 0.45:
                return True
        return False
    ...
    def compute_terminated(
        ...
        # Terminate if the agent is in a dangerous zone
        if self._is_in_dangerous_zone(achieved_goal):
            return True
    ...
    def update_target_site_pos(self): # <<< MODIFIED: Added pos argument to match superclass, but it's not used here.
